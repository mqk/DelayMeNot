...
def construct_datasets(Destination, min_year=2008, subsample=None):
    import MySQLdb
    ...
    ### flights origination from Destination
    print '   Querying database...'
    time0 = time.time()
    ## print "SELECT Year, Month, DayofMonth, DayOfWeek, CRSDepTime, CRSArrTime, UniqueCarrier, CRSElapsedTime, ArrDelay, Origin, Distance FROM flights WHERE Dest = '%s' AND Year >= %d;" % (Origin, min_year) 
    cur.execute("SELECT Year, Month, DayofMonth, DayOfWeek, CRSDepTime, CRSArrTime, UniqueCarrier, CRSElapsedTime, ArrDelay, Origin, Distance FROM flights WHERE Dest = '%s' AND Year >= %d;" % (Destination, min_year))        
    ...
    df = DataFrame(rows, columns=['Year', 'Month', 'DayOfMonth', 'DayOfWeek', 'ScheduledDepartureTime', 'ScheduledArrivalTime', 'Carrier', 'ScheduledElapsedTime', 'ArrivalDelay', 'Origin', 'Distance'])
    del rows
...
def batch_by_Destination():
    ...
    destinations = []
    destination_dict = {}
    f = open('destination_list.txt','rb')
    for line in f:
        if line.startswith('#'): continue
        toks = line.split()
        destinations.append(toks[0])
        destination_dict[toks[0]] = int(toks[1])
    f.close()
    ...
    for destination in destinations:
        ...
        time0 = time.time()
        save_to_pickle(data_train, data_test, filename=pickle_filename, gzip=True)
        dt = time.time() - time0
