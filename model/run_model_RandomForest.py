...
def run_model(origin, subsample=None, min_Nflights=None):

    ### Unpickle datasets
    print 'Unpickling datasets...'
    time0 = time.time()
    filename = '/data/DelayMeNot/data/pickles_by_origin/datasets_%s.pkl' % origin
    ...
    grouped = data_train.groupby('Destination')
    Nflights = grouped['Destination'].count()
    Nflights.sort(ascending=True)
    if min_Nflights:
        dest_list = list(Nflights[Nflights > min_Nflights].index)
        if len(dest_list) == 0:
            print 'Found no routes with more than %d flights!' % min_Nflights
            return 0
        data_train = data_train[data_train['Destination'].isin(dest_list)]
        data_test = data_test[data_test['Destination'].isin(dest_list)]
    else:
        dest_list = list(Nflights.index)
    ...
    dummies = pd.get_dummies(data_train['Destination'],prefix='Destination')
    data_train = data_train.join(dummies.ix[:,1:])
    dummies = pd.get_dummies(data_test['Destination'],prefix='Destination')
    data_test = data_test.join(dummies.ix[:,1:])

    ### Drop dummified columns
    data_train = data_train.drop(['Carrier','Destination'],axis=1)
    data_test = data_test.drop(['Carrier','Destination'],axis=1)
    ...
    filename = '../RandomForest_models/rfm_%s.pkl' % origin
    f = open(filename, 'wb')
...
if __name__ == "__main__":
    ...
    parser.add_argument('origin',
                        help='The 3-letter airport code of the origin')
    ...
    run_model(args.origin, subsample=args.subsample, min_Nflights=args.min_Nflights)
